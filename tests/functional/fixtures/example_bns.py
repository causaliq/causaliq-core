...
def x(check=None):  # single Gaussian X node
    if check is None:
        return BN(
            dag.x(), {"X": (LinGauss, {"mean": 0.0, "sd": 1.0, "coeffs": {}})}
        )

    assert isinstance(check, BN)
    dag.x(check.dag)
    assert check.cnds == {
        "X": LinGauss({"mean": 0.0, "sd": 1.0, "coeffs": {}})
    }
...
def xy(check=None):  # X --> Y  Gaussian network
    if check is None:
        return BN(
            dag.xy(),
            {
                "X": (LinGauss, {"mean": 2.0, "sd": 1.0, "coeffs": {}}),
                "Y": (
                    LinGauss,
                    {"mean": 0.5, "sd": 0.5, "coeffs": {"X": 1.5}},
                ),
            },
        )
    ...
    assert check.cnds == {
        "X": LinGauss({"mean": 2.0, "sd": 1.0, "coeffs": {}}),
        "Y": LinGauss({"mean": 0.5, "sd": 0.5, "coeffs": {"X": 1.5}}),
    }
...
def x_y(check=None):  # X  Y Gaussian network
    if check is None:
        return BN(
            dag.x_y(),
            {
                "X": (LinGauss, {"mean": 0.2, "sd": 0.1, "coeffs": {}}),
                "Y": (LinGauss, {"mean": -4.0, "sd": 2.0, "coeffs": {}}),
            },
        )
...
def xyz(check=None):  # X --> Y  --> Z Gaussian network
    if check is None:
        return BN(
            dag.xyz(),
            {
                "X": (LinGauss, {"mean": 0.0, "sd": 5.0, "coeffs": {}}),
                "Y": (
                    LinGauss,
                    {"mean": -0.7, "sd": 0.5, "coeffs": {"X": -1.2}},
                ),
                "Z": (
                    LinGauss,
                    {"mean": 0.03, "sd": 0.05, "coeffs": {"Y": 0.3}},
                ),
            },
        )
    ...
    assert check.cnds == {
        "X": LinGauss({"mean": 0.0, "sd": 5.0, "coeffs": {}}),
        "Y": LinGauss({"mean": -0.7, "sd": 0.5, "coeffs": {"X": -1.2}}),
        "Z": LinGauss({"mean": 0.03, "sd": 0.05, "coeffs": {"Y": 0.3}}),
    }
...
def xy_zy(check=None):  # X --> Y  <-- Z Gaussian network
    if check is None:
        return BN(
            dag.xy_zy(),
            {
                "X": (LinGauss, {"mean": -3.07, "sd": 0.45, "coeffs": {}}),
                "Y": (
                    LinGauss,
                    {"mean": -2.7, "sd": 1.5, "coeffs": {"X": 1.2, "Z": -0.4}},
                ),
                "Z": (LinGauss, {"mean": 6.2, "sd": 1.4, "coeffs": {}}),
            },
        )
