...
# Test rename fails with missing arguments
def test_bn_rename_type_error_1():
    bn = read_bn(str(TESTDATA_DIR / "ab.dsc"))
    with pytest.raises(TypeError):
        bn.rename()


# Test rename fails with invalid name_map types
def test_bn_rename_type_error_2():
    bn = read_bn(str(TESTDATA_DIR / "ab.dsc"))
    with pytest.raises(TypeError):
...
# Test rename fails with non-string keys in name_map
def test_bn_rename_type_error_3():
    bn = read_bn(str(TESTDATA_DIR / "ab.dsc"))
    with pytest.raises(TypeError):
...
# Test rename fails with non-string values in name_map
def test_bn_rename_type_error_4():
    bn = read_bn(str(TESTDATA_DIR / "ab.dsc"))
    with pytest.raises(TypeError):
...
# Test rename fails with non-existent node names
def test_bn_rename_value_error_1():
    bn = read_bn(str(TESTDATA_DIR / "ab.dsc"))
...
# Test rename changes first node name while keeping order
def test_bn_rename_bn_ab_1_ok():
    bn = read_bn(str(TESTDATA_DIR / "ab.dsc"))
    orig_bn = deepcopy(bn)
...
# Test rename changes first node name and alters order
def test_bn_rename_bn_ab_2_ok():
    bn = read_bn(str(TESTDATA_DIR / "ab.dsc"))
    orig_bn = deepcopy(bn)
...
# Test rename changes both node names and order
def test_bn_rename_bn_ab_3_ok():
    bn = read_bn(str(TESTDATA_DIR / "ab.dsc"))
    orig_bn = deepcopy(bn)
...
# Test rename on three-node network changes all names and order
def test_bn_rename_bn_ab_cb_1_ok():
...
# Test rename on Gaussian network keeps node order
def test_bn_rename_bn_xy_1_ok():
    ...
    print(
        "\n\nBN names changed to:\n{}\nCNDs:\n{}\n".format(
            bn.dag,
            "\n".join(["{}: {}".format(n, c) for n, c in bn.cnds.items()]),
        )
    )
...
# Test rename on Gaussian network changes node order
def test_bn_rename_bn_xy_2_ok():
    bn = read_bn(str(TESTDATA_DIR / "xy.xdsl"))
    orig_bn = deepcopy(bn)
...
# Test rename on three-node Gaussian network keeps order
def test_bn_rename_bn_xy_zy_1_ok():
...
# Test rename on three-node Gaussian network changes order
def test_bn_rename_bn_xy_zy_2_ok():
    bn = read_bn(str(TESTDATA_DIR / "xy_zy.xdsl"))
    orig_bn = deepcopy(bn)
