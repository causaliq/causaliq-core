...
def bn():
    ...
    df = df.astype(str)  # Ensure categorical data is treated as strings
    data = DataFrameAdapter(df)
    return BN.fit(dag, data)


# bad arg types
def test_bn_prob_lnprob_case_type_error(bn):
...
# bad arg values
def test_bn_prob_lnprob_case_value_error1(bn):
...
# bad node name in case
def test_bn_prob_lnprob_case_value_error2(bn):
    with pytest.raises(ValueError):
        bn.lnprob_case({"N3": "1", "N2": "1"})


# bad value in case
def test_bn_prob_lnprob_case_value_error3(bn):
    with pytest.raises(ValueError):
        bn.lnprob_case({"N1": "bad value", "N2": "1"})


# correct values for N=12 dataset
def test_bn_prob_lnprob_case_heckerman(bn):
    assert (
        values_same(
            10 ** (bn.lnprob_case({"N1": "1", "N2": "1"})), 0.25, sf=10
        )
        is True
    )
    assert (
        values_same(
            10 ** (bn.lnprob_case({"N1": "1", "N2": "2"})), 0.1666666667, sf=10
        )
        is True
    )
    assert (
        values_same(
            10 ** (bn.lnprob_case({"N1": "2", "N2": "1"})), 0.25, sf=10
        )
        is True
    )
    assert (
        values_same(
            10 ** (bn.lnprob_case({"N1": "2", "N2": "2"})), 0.3333333333, sf=10
        )
        is True
    )
...
# bad arg types
def test_bn_prob_marginal_distribution_type_error1(bn):
    with pytest.raises(TypeError):
...
# bad node value
def test_bn_prob_marginal_distribution_value_error1(bn):
    with pytest.raises(ValueError):
        bn.marginal_distribution("N3")


# bad parents value
def test_bn_prob_marginal_distribution_value_error2(bn):
    with pytest.raises(ValueError):
...
# test marginal distribution for single node N1
...
# test marginal distribution for single node N2
...
# test joint marginal distribution for N1, N2
...
# test conditional marginal distribution for N2 given N1
...
# test global distribution for heckerman dataset
    ...
    for n in bn.dag.nodes:
        print(
            "{}: {}".format(
                ...
                {
                    v: "{:.6f}".format(p)
                    for v, p in global_dist.groupby(n, observed=False)[""]
                    .sum()
                    .items()
                },
            )
...
# test marginal distributions with A->C<-B structure
def test_bn_prob_marginal_distribution_ac_bc():
    ...
    df = DataFrame(data, dtype="category")
    data = DataFrameAdapter(df)
    ...
    assert dicts_same(
        dict(bn.marginal_distribution("A")[""]),
        {"0": 0.2, "1": 0.2, "2": 0.3, "3": 0.3},
    )
    assert dicts_same(
        dict(bn.marginal_distribution("B")[""]),
    ...
    assert dicts_same(
        dict(bn.marginal_distribution("C")[""]),
        dict(gd.groupby("C", observed=False)[""].sum()),
    )
    ...
    for pvs in p:
        ...
        from_gd = (
            gd.loc[(gd["A"] == pvs[0]) & (gd["B"] == pvs[1]), ["C", ""]]
            .set_index("C")[""]
            .to_dict()
        )
...
# test global distribution for cancer network
...
# test marginal distributions for common effect structure
def test_bn_prob_marginal_distribution_common_effect():
    dag = DAG(
        ["Cause1", "Cause2", "CommonEffect"],
        [("Cause1", "->", "CommonEffect"), ("Cause2", "->", "CommonEffect")],
    )
    df = pd.read_csv(str(TESTDATA_DIR / "common_effect_1K.csv"))
    df = df.astype(str)  # Ensure categorical data is treated as strings
    data = DataFrameAdapter(df)
    bn = BN.fit(dag, data)
    ...
    assert dicts_same(
        bn.cnds["Cause1"].cdist(), {"no": 0.8, "yes": 0.2}, sf=10
    )
    assert dicts_same(
        bn.cnds["Cause2"].cdist(), {"no": 0.6, "yes": 0.4}, sf=10
    )
    cpt = bn.cnds["CommonEffect"]
    assert dicts_same(
        cpt.cdist({"Cause1": "no", "Cause2": "no"}),
        {"no": 0.7, "yes": 0.3},
        sf=10,
    )
    assert dicts_same(
        cpt.cdist({"Cause1": "no", "Cause2": "yes"}),
        {"no": 0.5, "yes": 0.5},
        sf=10,
    )
    assert dicts_same(
        cpt.cdist({"Cause1": "yes", "Cause2": "no"}),
        {"no": 0.4, "yes": 0.6},
        sf=10,
    )
    assert dicts_same(
        cpt.cdist({"Cause1": "yes", "Cause2": "yes"}),
        {"no": 0.6, "yes": 0.4},
        sf=10,
    )
...
# test marginal distributions for cancer network with generated data
def test_bn_prob_marginal_distribution_cancer():
    ...
    for node in bn.dag.nodes:
        ...
        for combo in md:
            ...
            query = " & ".join(
                ['{} == "{}"'.format(p[0], p[1]) for p in zip(others, combo)]
            )
