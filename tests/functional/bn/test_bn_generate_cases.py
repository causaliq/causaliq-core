...
def bn():
    dag = DAG(["N1", "N2"], [("N1", "->", "N2")])
    df = pd.read_csv(TESTDATA_DIR / "heckerman.csv", dtype="category")
...
@pytest.fixture(scope="function")  # temp file, automatically removed
def tmpfile():
    _tmpfile = str(TESTDATA_DIR / "tmp" / f"{int(random() * 10000000)}.csv")
    yield _tmpfile
    remove(_tmpfile)
...
def test_bn_generate_cases_value_error2(bn):  # non-existent directory
    with pytest.raises(FileNotFoundError):
        bn.generate_cases(2, str(TESTDATA_DIR / "nonexistent" / "bad.csv"))
...
def test_bn_generate_cases_cancer_write_ok(tmpfile):  # OK writing Cancer BN
    ...
    df_check = pd.read_csv(tmpfile, dtype="category")
    check = DataFrameAdapter(df_check)
    assert check.df.equals(cases)
...
def test_bn_generate_cases_xy_10k_ok():
    ...
    assert values_same(mean, 1.99981, sf=6)
