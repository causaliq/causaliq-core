...
from causaliq_core.utils import dicts_same, dists_same, values_same


# Test values_same with invalid argument types and counts
...
# Test values_same with invalid significant figures parameter
...
# Test values_same with small integer values including zeros
...
# Test values_same with medium-sized integer values
...
# Test values_same with large integer values
...
# Test values_same with small floating-point values
...
# Test values_same with medium-sized floating-point values
...
# Test values_same with large floating-point values
def test_metrics_values_same_large_floats():
    assert values_same(1.0004e10, 1.0e10) is True
    assert values_same(1.0004e10, 1.0e10, sf=4) is True
    assert values_same(1.0004e10, 1.0e10, sf=5) is False


# Test values_same with mixed integer and float values
...
# Test values_same with NaN values
...
# Test values_same with boolean values treated as 0/1
...
# Test values_same with zero significant figures
...
# Test values_same with boolean significant figures parameter
...
# Test dicts_same with invalid argument types and counts
...
# Test dicts_same with different keys in strict mode
...
# Test dicts_same with different keys in non-strict mode
...
# Test dicts_same with values that should compare as equal
...
# Test dicts_same with numeric and None value mismatches
...
# Test dists_same with no arguments
def test_metrics_dists_same_type_error_1():  # no arguments
    with pytest.raises(TypeError):
        dists_same()


# Test dists_same with insufficient arguments
...
# Test dists_same with invalid argument types
...
# Test dists_same by comparing univariate distribution with itself
def test_metrics_dists_same_ok_us1_():  # compare univariate with itself
    ...
    print(
        "\nUnivariate:\n{}\n{}SAME (to {} s.f.) as:\n{}".format(
            dist1, "" if same else "NOT ", sf, dist1
        )
    )
    assert same


# Test dists_same with identical univariate distribution copies
def test_metrics_dists_same_ok_us2_():  # identical univariate copies
    ...
    print(
        "\nUnivariate:\n{}\n{}SAME (to {} s.f.) as:\n{}".format(
            dist1, "" if same else "NOT ", sf, dist2
        )
    )
    assert same


# Test dists_same with identical univariate distributions
...
# Test dists_same with almost identical univariate distributions
...
# Test dists_same with nearly identical univariate distributions
...
# Test dists_same with high-precision univariate distributions
...
# Test dists_same with different primary variable names
...
# Test dists_same with different secondary variable names
...
# Test dists_same with different primary variable values
...
# Test dists_same with different number of primary variable values
...
# Test dists_same by comparing bivariate distribution with itself
def test_metrics_dists_same_ok_11_():  # compare bivariate with itself
    dist1 = DataFrame(
        {("0",): {"0": 0.2, "1": 0.3}, ("1",): {"0": 0.4, "1": 0.1}}
    )
    dist1.index.name = "B"
...
def test_metrics_dists_same_different_secondary_values(capsys):
    """Test dists_same with different secondary values (line 145-146)."""
    # Create distributions with different secondary column values
    dist1 = DataFrame(
        {("0",): {"0": 0.2, "1": 0.3}, ("1",): {"0": 0.4, "1": 0.1}}
    )
    dist1.index.name = "B"
    ...
    # Capture the print output to verify the correct path was taken
    captured = capsys.readouterr()
    assert "dists_same: different secondary values" in captured.out
    assert not same


def test_metrics_dists_same_different_probabilities(capsys):
    """Test dists_same with different probabilities (line 150-151)."""
    # Create distributions with same structure but different probabilities
    dist1 = DataFrame(
        {("0",): {"0": 0.2, "1": 0.3}, ("1",): {"0": 0.4, "1": 0.1}}
    )
    dist1.index.name = "B"
    ...
    dist2 = DataFrame(
        {("0",): {"0": 0.2, "1": 0.3}, ("1",): {"0": 0.9, "1": 0.1}}
    )  # Different probability
    dist2.index.name = "B"
    dist2.columns.names = ["A"]

    sf = 1  # Low precision to ensure difference is detected
    same = dists_same(dist1, dist2, sf)

    # Capture the print output to verify the correct path was taken
    captured = capsys.readouterr()
    assert "dists_same: different probabilities" in captured.out
    assert not same
