...
- dict_to_adjmat() function for adjacency matrix creation
"""
...
def test_adjmat_type_errors():
    # Test with non-dict
    with pytest.raises(
        TypeError, match="dict_to_adjmat called with bad arg type"
    ):
        dict_to_adjmat([1, 2, 3])

    # Test with dict but non-list values
    with pytest.raises(
        TypeError, match="dict_to_adjmat called with bad arg type"
    ):
        dict_to_adjmat({"A": "not_a_list"})

    # Test with dict of lists but non-int values
    with pytest.raises(
        TypeError, match="dict_to_adjmat called with bad arg type"
    ):
        dict_to_adjmat({"A": ["not_an_int"]})


def test_adjmat_value_errors():
    # Test with wrong column length
    with pytest.raises(
        ValueError, match="some columns wrong length for dict_to_adjmat"
    ):
        dict_to_adjmat({"A": [0, 1], "B": [0, 1, 2]})  # Mismatched lengths

    # Test with invalid edge type values (correct column lengths)
    with pytest.raises(
        ValueError, match="invalid integer values for dict_to_adjmat"
    ):
        dict_to_adjmat(
            {"A": [99, 0], "B": [0, 99]}
        )  # 99 is not a valid edge type
...
def test_legacy_adjmat_type_error_no_args():
    # Test calling dict_to_adjmat() with no arguments
    with pytest.raises(
        TypeError, match="dict_to_adjmat called with bad arg type"
    ):
        dict_to_adjmat()
...
def test_legacy_adjmat_value_error_wrong_lengths():
    ...
    with pytest.raises(
        ...
        dict_to_adjmat({"A": [0, 1]})  # 2 elements but only 1 column
    with pytest.raises(
        ValueError, match="some columns wrong length for dict_to_adjmat"
    ):
        dict_to_adjmat(
...
def test_legacy_adjmat_value_error_invalid_edge_values():
    ...
    with pytest.raises(
        ...
        dict_to_adjmat({"A": [7]})  # 7 is not a valid edge type
    with pytest.raises(
        ValueError, match="invalid integer values for dict_to_adjmat"
    ):
        dict_to_adjmat({"A": [-1]})  # -1 is not a valid edge type
    with pytest.raises(
        ValueError, match="invalid integer values for dict_to_adjmat"
    ):
        dict_to_adjmat({"A": [0, 1], "B": [-99, 0]})  # -99 is not valid
