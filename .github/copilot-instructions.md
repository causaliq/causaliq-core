...
### Terminal Command Behaviour
- **Do NOT issue follow-up commands** (like `echo "done"`) to check if a
  long-running command has finished - this interrupts the current command
- Wait for terminal output to return naturally
- If output appears truncated, the command is likely still running
